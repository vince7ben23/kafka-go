...
The broker logic is split across four files under `app/`:

- **`app/main.go`** — `Server` struct, `NewServer()`, `Run()` accept loop, `handleRequest()` per-connection request loop, `writeResponse()`
- **`app/request.go`** — `Request` struct, `parseRequest()` binary decoder
- **`app/response.go`** — `Encoder` interface, `Response`, `ApiVersionsResponse`, `ApiVersion` types, `NewResponse()` factory, `createApiVersionsResponse()`
- **`app/main_test.go`** — unit and integration tests covering request parsing, response encoding, and end-to-end connection handling
...
| *       | (others)    | —        | Returns base `Response` with only CorrelationID |

### Key Design Patterns

- **`Encoder` interface** — strategy pattern; `Response` and `ApiVersionsResponse` each implement `Encode() []byte`
