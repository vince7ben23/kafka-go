...
# Fetch Request v16 — 單一「已知且含一則訊息」的 topic_id
# 用來手動驗證 stage:topic_id 存在於 cluster metadata,且該 topic 的 partition 0
# 在磁碟上有一則訊息。server 應回 top-level error_code=0、throttle_time_ms=0,
# responses 有 1 個 topic(topic_id 與送出值相同),其下 1 個 partition:
# partition_index=0、error_code=0x0000 (0, 成功),records 為磁碟 log 檔的完整 bytes。
#
# 前置:產生「已知 topic + partition 訊息 log」的 fixture,把 metadata fixture 放到
# broker 讀取的路徑(訊息 log 已直接寫到正式路徑,不需 rename),再啟動 broker:
#   go run ./tools/gen_test_logs -case fetch-single-message
#   cp /tmp/kraft-combined-logs/__cluster_metadata-0/fetch_single_message_meta.log \
#      /tmp/kraft-combined-logs/__cluster_metadata-0/00000000000000000000.log
#   ./your_program.sh
#
# 驗證時可對照磁碟上的訊息 log 內容:
#   hexdump -C /tmp/kraft-combined-logs/test-topic-0/00000000000000000000.log
# 回應中 records 段的 bytes 應與此檔完全相同。
#
